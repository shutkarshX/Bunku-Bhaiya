...
def count_scheduled_classes(start_value, end_value, today_remaining):
    """Return scheduled classes in an inclusive date range.

    Today's portal-unposted classes use the live remaining count. Future
    teaching days use the academic calendar's normal eight classes/day.
    Past dates are not accepted by the planner UI, but are safely ignored.
    """
    try:
    ...
    while cursor <= end:
        if cursor < today:
            ...
            continue
        if cursor == today:
            total += max(0, int(today_remaining))
        elif is_teaching_day(cursor.isoformat()):
            total += TEACHING_CLASSES_PER_DAY
        cursor += timedelta(days=1)
    return total
...
def scenario():
    """Return a non-persistent attendance planning simulation."""
    attendance_data = get_user_attendance()
    ...
    plan_from = payload.get("plan_from", date.today().isoformat())
    plan_to = payload.get("plan_to", plan_from)
    planned_attended = payload.get("planned_attended", 0)
    planned_bunked = payload.get("planned_bunked", classes_missed)

    subjects = attendance_data.get("subjects") or []
    try:
        portal_today_remaining = max(0, int(subjects[0].get("_bunkmaster_remaining_today", 0) or 0)) if subjects else 0
    except (TypeError, ValueError):
        portal_today_remaining = 0
    try:
        today_override = get_today_override()
        live_today_remaining = portal_today_remaining if today_override is None else today_override
    except Exception:
        live_today_remaining = portal_today_remaining

    planner_period_classes = count_scheduled_classes(plan_from, plan_to, live_today_remaining)
    if planner_period_classes <= 0:
        planner_period_classes = live_today_remaining

    # Keep today's event logic and the date-range planner in one calculation.
    result = calculate_scenario(
        attendance_data,
        phase_1_result,
        "today",
        classes_missed,
        today_remaining_override=today_override,
        event_mode=event_mode,
        event_attended=event_attended,
        planner_period_classes=planner_period_classes,
        planned_attended=planned_attended,
        planned_bunked=planned_bunked,
    )
